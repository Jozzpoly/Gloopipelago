...
const BrowserSimulation=createM4Simulation(Simulation);
...
let W=1200,H=900,DPR=1,view=computeViewTransform(W,H),paused=false,speedMul=1,acc=0;
...
function startWorld(nextSeed=seed){
  seed=nextSeed>>>0;
  sim=new BrowserSimulation({seed});
...
function draw(){
  ...
  ui.overlay.innerHTML=`seed <b>${seed}</b> · habitaty <b>${M4_HABITATS.length}</b> · nisze <b>${nicheA}/${generalists}/${nicheB}</b><br><span style="color:#8f9aa5">A/B × dense/diffuse · specjalista A / generalista / specjalista B</span>`;
}
