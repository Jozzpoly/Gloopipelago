...
function schedulerRun({ fps, speedMul, wallSeconds }) {
  ...
  return {
    fps,
    targetMultiplier: speedMul,
    wallSeconds,
    frames,
    steps,
    simulatedSeconds,
    achievedMultiplier: simulatedSeconds / wallSeconds,
    backlogSeconds: acc,
  };
}
...
function debtScenario() {
  ...
  const phases = [];
  for (const phase of [
    { label:'40x-request', speedMul:40, wallSeconds:10 },
    { label:'1x-after-40x', speedMul:1, wallSeconds:20 },
  ]) {
    const startAcc = acc;
    const startSim = sim;
    const frames = Math.round(fps * phase.wallSeconds);
    for (let i = 0; i < frames; i++) {
      const r = currentSchedulerFrame(acc, 1/fps, phase.speedMul);
      acc = r.acc;
    ...
    phases.push({
      ...phase,
      fps,
      simulatedSeconds: sim - startSim,
      achievedMultiplier: (sim - startSim) / phase.wallSeconds,
      backlogStartSeconds: startAcc,
      backlogEndSeconds: acc,
    });
  }
  return phases;
}

function runExact(seed, totalTicks, batchSize) {
  const sim = new Simulation({ seed, width:900, height:700, ...BROWSER_CONFIG });
  let tick = 0;
  while (tick < totalTicks) {
    const n = Math.min(batchSize, totalTicks - tick);
    for (let i = 0; i < n; i++) sim.step(FIXED_DT);
    tick += n;
  }
  return {
    seed: seed >>> 0,
...
function benchmarkNatural(seed, warmupTicks = 1200, measuredTicks = 2400) {
  ...
  return {
    kind:'natural', seed:seed>>>0, warmupTicks, measuredTicks, ms,
    stepsPerSecond,
    equivalentRealtimeMultiplier: stepsPerSecond / 60,
    start:{time:start.time,population:start.population,food:start.food,maxGeneration:start.maxGeneration},
    end:{time:end.time,population:end.population,food:end.food,maxGeneration:end.maxGeneration},
  };
}
...
function syntheticHeavy(seed = 0xC0FFEE) {
  ...
  sim.blobs = Array.from({length:200},(_,i)=>({
    ...
    vx:((i%11)-5)*1.7,vy:((i%13)-6)*1.4,a:(i%31)/31*Math.PI*2,
    energy:1e9,age:0,generation:i%14,reproCooldown:1e9,
  ...
  sim.foods = Array.from({length:500},(_,i)=>({
    x:(71*i+17)%900,y:(47*i+19)%700,e:6+(i%13),rich:false,kind:i%2,expiresAt:1e12
  }));
...
function throughput() {
  return {
    natural:[1,0x12345678,956866913].map(seed => benchmarkNatural(seed)),
    heavy:benchmarkHeavy(),
    note:'Node/V8 core throughput only; browser rendering/compositing are not included.'
  };
}
...
const receipt = {
  format:'gloopipelago-fast-forward-readiness-audit',
  schema:1,
  currentScheduler:{guardSteps:GUARD_STEPS,rawClampSeconds:RAW_CLAMP_SECONDS,fixedDt:FIXED_DT,matrix,debtScenario:debtScenario()},
  fixedStepBatching:batchEquivalence(),
  throughput:throughput(),
  interpretation:{
    x40CurrentRafLoop:'NOT CAPABLE RELIABLY',
    fixedDtBatching:'EXACT FOR EQUIVALENT STEP ORDER',
    nextEvidenceNeed:'qualify a decoupled fast-forward scheduler with achieved-speed reporting and browser responsiveness',
  }
};
