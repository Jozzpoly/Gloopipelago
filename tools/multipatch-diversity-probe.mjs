...
function multiPatch(sim,table,kind=null){const k=kind??(sim.rng()<.5?0:1),list=table[k],c=list[Math.floor(sim.rng()*list.length)],spread=Math.min(sim.width,sim.height)*.075,clamp=(v,a,b)=>Math.max(a,Math.min(b,v));return{x:clamp(c[0]*sim.width+sim.rand(-spread,spread),8,sim.width-8),y:clamp(c[1]*sim.height+sim.rand(-spread,spread),8,sim.height-8),kind:k}}
class FourPatchSimulation extends Simulation{foodPatch(kind=null){return multiPatch(this,PATCHES.four,kind)}}
class EightPatchSimulation extends Simulation{foodPatch(kind=null){return multiPatch(this,PATCHES.eight,kind)}}
const profiles=[
  {name:'four-patches-900',Class:FourPatchSimulation,width:900,height:700,foodRate:10},
  {name:'eight-patches-900',Class:EightPatchSimulation,width:900,height:700,foodRate:10},
  {name:'eight-patches-1200-scaled',Class:EightPatchSimulation,width:1200,height:900,foodRate:16,extraFounders:34,extraFood:145}
];
